...
# Make the repository root importable when this file is launched directly by
# an MCP client from an arbitrary working directory.
REPOSITORY_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(REPOSITORY_ROOT))
from analysis.tiff_tilt import correct_tiff_tilt as run_tiff_tilt_correction
from analysis.tiff_skeletonization import skeletonize_tiff_slices
...
@mcp.tool()
def skeletonize_tiff(
    input_filepath: str,
    output_filepath: str,
) -> dict:
    """
    Skeletonize every 2D page of an already-segmented TIFF stack.

    Any nonzero input pixel is foreground. Pages are processed independently
    and streamed to a new uint8 TIFF where background is 0 and skeleton pixels
    are 255. The input is never overwritten, and partial output is removed if
    processing fails.

    Args:
        input_filepath: Source segmented .tif or .tiff stack.
        output_filepath: Different .tif or .tiff path for the skeleton stack.

    Returns:
        A structured result containing output path, page count, and pixel
        statistics, or a structured error result.
    """
    try:
        return skeletonize_tiff_slices(input_filepath, output_filepath)
    except Exception as exc:
        return {
            "status": "error",
...
def correct_tiff_tilt(
    ...
    """
    Estimate and correct pitch and roll in a binary lattice TIFF stack.

    Repeating internal lattice lines in the centered portion of the stack are
    used to estimate Z-Y and Z-X tilt. The stack is corrected with one
    nearest-neighbor 3D affine transform, preserving its shape, datatype, and
    binary values. The output is committed only after its residual tilt has
    been verified.

    Args:
        input_filepath: Source multi-page binary .tif or .tiff stack.
        output_filepath: Different path for the corrected TIFF stack.
        max_angle_degrees: Maximum absolute angle to search on either axis.
        middle_fraction: Centered fraction of Z slices used for estimation.
        voxel_size_z: Physical spacing between TIFF pages.
        voxel_size_y: Physical pixel spacing along image rows.
        voxel_size_x: Physical pixel spacing along image columns.

    Returns:
        A structured success result containing estimated, applied, and
        residual angles, or a structured error result.
    """
    try:
        result = run_tiff_tilt_correction(
            input_filepath,
            output_filepath,
            max_angle=max_angle_degrees,
            middle_fraction=middle_fraction,
            voxel_spacing=(voxel_size_z, voxel_size_y, voxel_size_x),
        )
